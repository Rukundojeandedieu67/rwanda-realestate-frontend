...
export default function SuperAdminSettingsPage() {
  ...
  async function save(event: React.FormEvent) {
    ...
    try {
      ...
      if (settings.smtp_from_address) editableSettings.smtp_from_address = settings.smtp_from_address
      if (settings.smtp_from_name) editableSettings.smtp_from_name = settings.smtp_from_name
    ...
    <form onSubmit={save} className="space-y-6">
      ...
      <section className="rounded-xl border border-amber-300/30 bg-slate-900 p-6">
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          {([['smtp_host', 'SMTP host'], ['smtp_port', 'SMTP port'], ['smtp_username', 'SMTP username'], ['smtp_encryption', 'Encryption'], ['smtp_from_address', 'From email'], ['smtp_from_name', 'From name'], ['smtp_password', 'SMTP password']] as const).map(([field, label]) => {
            const configured = Boolean(settings[`${field}_configured` as keyof SiteSettings]) || Boolean(settings[field])
            return <label key={field}><span className="mb-2 flex items-center justify-between text-sm font-semibold text-slate-300"><span>{label}</span><span className={configured ? 'text-emerald-300' : 'text-slate-500'}>{configured ? 'Configured' : 'Not set'}</span></span>{field === 'smtp_encryption' ? <select value={String(settings[field] || 'smtp')} onChange={event => update(field, event.target.value)} className="w-full rounded-lg border border-slate-600 bg-slate-950 px-3 py-2 text-white"><option value="smtp">TLS / STARTTLS (587)</option><option value="smtps">SSL / implicit TLS (465)</option><option value="">None</option></select> : <input type={field === 'smtp_password' ? 'password' : 'text'} value={String(settings[field] || '')} onChange={event => update(field, event.target.value)} placeholder={configured ? 'Leave blank to keep current value' : ''} className="w-full rounded-lg border border-slate-600 bg-slate-950 px-3 py-2 text-white" />}</label>
          })}
        </div>
      </section>
      <section className="rounded-xl border border-slate-700 bg-slate-900 p-6">
