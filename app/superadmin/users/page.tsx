...
export default function SuperAdminUsersPage() {
  ...
  async function loadUsers() {
    setLoading(true)
    try {
      setUsers(await api.superadmin.users({ search: query, role }))
      setError(null)
    } catch (err: any) {
  ...
  async function act(user: ManagedUser, action: 'promote' | 'demote' | 'deactivate' | 'activate') {
    const labels = { promote: 'promote this user to Admin', demote: 'demote this user', deactivate: 'deactivate this user', activate: 'reactivate this user' }
    ...
    try {
      const updated = action === 'deactivate'
        ? await api.superadmin.deactivateUser(user.id)
        : action === 'activate'
          ? await api.superadmin.activateUser(user.id)
          : await api.superadmin.updateUserRole(user.id, action === 'promote' ? 'admin' : 'user')
  ...
  const inactiveUsers = visibleUsers.filter(user => user.is_active === false && user.role !== 'superadmin')
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        <input value={query} onChange={event => setQuery(event.target.value)} onKeyDown={event => { if (event.key === 'Enter') void loadUsers() }} placeholder="Search name or email" className="flex-1 rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-white" />
        <select value={role} onChange={event => setRole(event.target.value)} className="rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-white"><option value="">All roles</option><option value="admin">Admin</option><option value="user">User</option><option value="superadmin">Super Admin</option></select>
        <button type="button" onClick={() => void loadUsers()} className="rounded-lg border border-slate-600 px-4 py-2 font-semibold text-slate-200 hover:bg-white/10">Search</button>
      </div>
